...
volatile bool	RFM69::sPacketSent;
...
void RFM69::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
 ...
 // while ((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00); // wait for ModeReady
  ...
  while (!sPacketSent && millis() - txStart < RF69_TX_LIMIT_MS); // wait for DIO0 to turn HIGH signalling transmission finish
...
void RFM69::interruptHandler() {
  ...
  {
	// readReg(REG_TESTLNA);	// For debugging to show that we got here
	sPacketSent = true;
  }
  RSSI = readRSSI();
  //digitalWrite(4, 0);
...
{
	RFM69::_inISR = true;
	RFM69::selfPointer->interruptHandler();
	RFM69::_inISR = false;
}
#else
